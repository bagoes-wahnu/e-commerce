...
class CartController extends Controller
{
    private function getCarts(){
        $carts = json_decode(request()->cookie('ks-carts'), true);
        // $carts = $this->getCarts();
        $carts = $carts != '' ? $carts:[];
        return $carts;
    }
    public function addToCart(Request $request){
        ...
        //BUAT COOKIE-NYA DENGAN NAME KS-CARTS
        //JANGAN LUPA UNTUK DI-ENCODE KEMBALI, DAN LIMITNYA 2800 MENIT ATAU 48 JAM
        $cookie = cookie('ks-carts', json_encode($carts), 2880);
    ...
    public function processCheckout(Request $request){
        ...
        try {
            ...
            $affiliate = json_decode(request()->cookie('ks-afiliasi'), true);
            // $affiliate = json_decode($request->cookie('ks-afiliasi'), true);
            dd($affiliate);
            ...
            Cookie::queue(Cookie::forget('ks-afiliasi'));
